...
mod tests {
    ...
    #[test]
    fn test_ctr256_large_chunked_matches_one_shot() {
        let key = test_key();
        let iv = test_iv();
        let data: Vec<u8> = (0..(512 * 1024 + 31)).map(|i| (i & 0xff) as u8).collect();
    ...
    #[test]
    fn test_add_counter_propagates_past_lower_half() {
        let mut iv = [0u8; 16];
        iv[8..16].fill(u8::MAX);
        ...
        assert!(result[8..16].iter().all(|byte| *byte == 0));
    }
